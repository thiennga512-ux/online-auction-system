...
public class HomeController implements LifecycleAwareController {
  ...
  private VBox createAuctionCard(Dto.AuctionCardDto session) {
    // ===== CONTAINER CHÍNH: VBox với CSS class hiện đại =====
    VBox card = new VBox(10);
    card.getStyleClass().add("auction-card");
    card.setPrefWidth(260);
    card.setMaxWidth(260);

    // ===== 1. ẢNH SẢN PHẨM (Wrapper + ImageView) =====
    // Dùng StackPane làm container để có thể clip ảnh bo góc
    javafx.scene.layout.StackPane imageWrapper = new javafx.scene.layout.StackPane();
    imageWrapper.getStyleClass().add("image-container");

    javafx.scene.image.ImageView imgView = new javafx.scene.image.ImageView();
    // Giới hạn kích thước khung hình - ảnh luôn nằm gọn trong thẻ
    imgView.setFitWidth(220);
    imgView.setFitHeight(140);
    // Giữ tỉ lệ ảnh gốc - không bao giờ bị méo hình hay phình to
    imgView.setPreserveRatio(true);
    // Làm mượt ảnh khi co dãn
    imgView.setSmooth(true);
    loadImage(imgView, session.imageUrl());

    // Clip hình chữ nhật bo góc 8px để ảnh không nhô ra ngoài container
    ...
    nameLabel.getStyleClass().add("auction-card__title");
    // Giới hạn 2 dòng bằng cách clamp max-height (44px = 2 dòng * 22px line-height)
    nameLabel.setMaxHeight(44);
    nameLabel.setWrapText(true);

    // ===== 4. MÔ TẢ SẢN PHẨM (tóm tắt) =====
    ...
    descLabel.getStyleClass().add("auction-card__description");
    descLabel.setWrapText(true);

    // ===== 5. NGƯỜI BÁN =====
    Label sellerLabel = new Label("👤 " + session.sellerName());
    sellerLabel.getStyleClass().add("auction-card__seller");

    // ===== 6. HIỆN NGƯỜI DẪN ĐẦU =====
    Label leaderLabel = buildLeaderLabel(session, status);

    // ===== 7. KHỐI GIÁ HIỆN TẠI (VBox riêng biệt) =====
    String formattedPrice = String.format("%,.0f", session.currentPrice());
    VBox priceBlock = new VBox(4);
    priceBlock.getStyleClass().add("price-block");
    priceBlock.setMaxWidth(Double.MAX_VALUE);

    Label priceLabelSmall = new Label("GIÁ HIỆN TẠI");
    priceLabelSmall.getStyleClass().add("price-block__label");

    Label priceValueLabel = new Label(formattedPrice + " VND");
    priceValueLabel.getStyleClass().add("price-block__value");

    priceBlock.getChildren().addAll(priceLabelSmall, priceValueLabel);
    ...
    // ===== 9. THỜI GIAN MỞ / KẾT THÚC (dòng phụ nhỏ) =====
    ...
    actionRow.getStyleClass().add("auction-action-row");

    // ===== GHÉP TẤT CẢ VÀO CARD =====
   ...
   * Sử dụng CSS class chuẩn: .auction-badge + .auction-badge--running / .auction-badge--open
   */
  private Label createStatusBadge(String status) {
    Label badge = new Label();
    badge.getStyleClass().add("auction-badge");
    if ("RUNNING".equals(status)) {
      badge.setText("ĐANG ĐẤU GIÁ");
      badge.getStyleClass().add("auction-badge--running");
    } else {
      badge.setText("SẮP MỞ");
      badge.getStyleClass().add("auction-badge--open");
    }
    return badge;
  }

  /**
   * Tạo Label đếm ngược chính với CSS class.
   * - OPEN    → "Mở sau: X giờ Y phút Z giây"  (countdown-label--open)
   * - RUNNING → "Kết thúc sau: X phút Y giây"   (countdown-label--running)
   */
  private Label buildCountdownLabel(String status, LocalDateTime startTime, LocalDateTime endTime) {
    Label label = new Label();
    label.getStyleClass().add("countdown-label");
  ...
  private void refreshCountdownLabel(Label label, String status,
    ...
    // Reset style classes — giữ lại .countdown-label, xóa class variant
    label.getStyleClass().removeAll("countdown-label--running", "countdown-label--open", "countdown-label--ended");

    if ("RUNNING".equals(status)) {
      LocalDateTime now = LocalDateTime.now();
      if (now.isAfter(endTime)) {
        label.setText("⏱ Đã kết thúc");
        label.getStyleClass().add("countdown-label--ended");
      } else {
        label.setText("⏱ Kết thúc sau: " + TimeFormatUtil.formatRemainingTime(endTime));
        label.getStyleClass().add("countdown-label--running");
      }
    } else {
      ...
      if (now.isAfter(startTime)) {
        ...
        label.getStyleClass().add("countdown-label--open");
      } else {
  ...
  private Label buildScheduleLabel(String status, LocalDateTime startTime, LocalDateTime endTime) {
    ...
    lbl.getStyleClass().add("auction-card__schedule");
  ...
  private Label buildLeaderLabel(Dto.AuctionCardDto session, String status) {
    ...
    if ("RUNNING".equals(status) && session.currentWinnerName() != null
      ...
      label.getStyleClass().add("auction-card__leader");
    } else {
      label.setText("👑 Chưa có ai đặt giá");
      label.getStyleClass().add("auction-card__leader--empty");
    }
    return label;
  }
  ...
  private void goToAuctionDetail(String sessionId) {
    if (sessionId == null || sessionId.isBlank()) {
      showError("Lỗi", "Không xác định được phiên đấu giá.");
      return;
    }
    SessionManager.getInstance().setSelectedAuctionSessionId(sessionId);
    MainController main = ClientMain.getMainController();
    if (main == null) {
      showError("Lỗi", "Giao diện chưa sẵn sàng. Vui lòng thử lại.");
      return;
    }
    main.switchContent("auction_detail.fxml");
  }
