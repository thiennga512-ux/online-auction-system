...
public class AuctionService {
   ...
   * @param sessionId ID phiên
   */
  public void cancelAuction(com.auction.common.model.user.User requester, String sessionId) {
    AuctionSession session = getSessionOrThrow(sessionId);
    ...
    boolean isAdmin = requester instanceof com.auction.common.model.user.Admin;
  ...
  public void startAuction(String sessionId) {
    ...
    try {
      ...
      Bid initialBid = Bid.createInitial(sessionId, session.getItem().getBasePrice());
      bidDAO.save(initialBid);

      System.out.printf("[AuctionService] Phiên %s đã BẮT ĐẦU! Giá khởi điểm: %.0f VND%n",
          sessionId.substring(0, 8), session.getItem().getBasePrice());

      // Broadcast sự kiện BẮT ĐẦU cho người xem chi tiết
      Dto.AuctionStartedEvent event = new Dto.AuctionStartedEvent(sessionId, session.getItem().getBasePrice());
  ...
  public void finishAuction(String sessionId) {
    ...
    try {
      ...
      // Broadcast sự kiện KẾT THÚC cho người xem chi tiết
  ...
  private void handleAuctionWin(AuctionSession session) throws SQLException {
    ...
    double commission = session.calculateCommissionFee(); // Gọi polymorphism!
    ...
    Optional<com.auction.common.model.user.User> sellerOpt = userDAO.findById(session.getSellerId());
    if (sellerOpt.isPresent() && sellerOpt.get() instanceof Seller seller) {
      seller.addEarnings(sellerReceives);
      ...
      com.auction.server.network.ClientHandler sellerHandler = com.auction.server.network.UserConnectionManager.getInstance().getHandler(seller.getId());
      if (sellerHandler != null) {
          Dto.DepositResultResponse result = new Dto.DepositResultResponse(seller.getBalance());
          sellerHandler.sendResponse(Response.success(ActionType.SYNC_BALANCE, "Thanh toán thành công từ phiên đấu giá", result));
      }
    }

    // Gửi email cho Người thắng (Bidder)
    Optional<com.auction.common.model.user.User> winnerOpt = userDAO.findById(session.getCurrentWinnerId());
    if (winnerOpt.isPresent() && winnerOpt.get() instanceof Bidder winner) {
      ...
      winner.deductFrozenBalance(finalPrice);
      ...
      com.auction.server.network.ClientHandler winnerHandler = com.auction.server.network.UserConnectionManager.getInstance().getHandler(winner.getId());
      if (winnerHandler != null) {
          double bal = (winner instanceof Seller s) ? s.getBalance() : winner.getDepositBalance();
          Dto.DepositResultResponse result = new Dto.DepositResultResponse(bal);
          winnerHandler.sendResponse(Response.success(ActionType.SYNC_BALANCE, "Thanh toán thành công phiên đấu giá", result));
      }
    }
    ...
    System.out.printf(
        ...
        finalPrice,
        session.getItem().getCategory().getDisplayName(),
        commission,
        sellerReceives);
  }
  ...
  public Bid placeBid(Bidder bidder, String sessionId, double amount) {
    ...
    synchronized (lock) {
      ...
      if (!item.isBidValid(session.getCurrentPrice(), amount)) {
        throw new IllegalArgumentException(
            String.format("Giá đặt %.0f VND không hợp lệ. Tối thiểu: %.0f VND",
                amount, item.getNextMinimumBid(session.getCurrentPrice())));
      }
      ...
      try {
        ...
        if (oldWinnerId != null) {
          Optional<com.auction.common.model.user.User> oldWinnerOpt = userDAO.findById(oldWinnerId);
          if (oldWinnerOpt.isPresent() && oldWinnerOpt.get() instanceof Bidder oldWinner) {
            // Chỉ giải phóng nếu người cũ khác người mới
            oldWinner.unfreezeBalance(oldPrice);
            ...
            if (oldWinnerHandler != null && !(oldWinner instanceof Seller)) {
                Dto.DepositResultResponse result = new Dto.DepositResultResponse(oldWinner.getDepositBalance());
                oldWinnerHandler.sendResponse(Response.success(ActionType.SYNC_BALANCE, "Đã hoàn trả số dư đóng băng", result));
            }
          }
        }
        ...
        if (bidderHandler != null && !(bidder instanceof Seller)) {
            Dto.DepositResultResponse result = new Dto.DepositResultResponse(bidder.getDepositBalance());
            bidderHandler.sendResponse(Response.success(ActionType.SYNC_BALANCE, "Đã đóng băng số dư", result));
        }
        ...
        bidder.recordBidPlaced(sessionId);
        userDAO.updateBidderDetails(bidder);
