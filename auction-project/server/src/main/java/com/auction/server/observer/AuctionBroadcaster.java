...
public class AuctionBroadcaster {
    ...
    public void subscribe(String sessionId, ClientObserver observer) {
        ...
        System.out
                .println("[Broadcaster] User " + observer.getUserId() + " theo dõi phiên " + sessionId.substring(0, 8));
    }
        ...
        if (list != null && !list.isEmpty()) {
            for (ClientObserver observer : list) {
                try {
                    ...
                    System.err.println("[Broadcaster] Lỗi khi gửi update đến user " + observer.getUserId() + ": "
                            + e.getMessage());
                }
            }
            System.out.println(
                    "[Broadcaster] Đã broadcast tới " + list.size() + " client cho phiên " + sessionId.substring(0, 8));
        }
    }

    public void broadcastToSession1(String sessionId, Response eventResponse) {
        List<ClientObserver> list = sessionSubscribers.get(sessionId);
        System.out.println("[Debug] Đang tìm người nhận cho session: " + sessionId);
        if (list != null) {
            System.out.println("[Debug] Số lượng người nhận tìm thấy: " + list.size());
            // ... vòng lặp gửi ...
        } else {
            System.out.println("[Debug] KHÔNG CÓ AI đăng ký session này!");
        }
    }
}
