...
public class SocketServer {
    ...
    private static final int CORE_THREADS = 50;// so luong thread co san de xu li ket noi, neu het se tao them den
                                               // MAX_THREADS neu can, neu van het se cho den khi co thread xong viec
    private static final int MAX_THREADS = 300;// so luong thread toi da co the tao de xu li ket noi, neu het se tu cho
                                               // den khi co thread xong viec
    private static final int QUEUE_CAPACITY = 2000;// so luong ket noi toi da co the cho vao hang doi de xu li, neu het
                                                   // se tu cho den khi co thread xong viec
    ...
    public void start() {
        ...
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {
            while (true) {
                ...
                // Giao việc đọc/ghi cho ThreadPool xử lý -> ServerSocket tiếp tục accept người
                // mới lập tức
