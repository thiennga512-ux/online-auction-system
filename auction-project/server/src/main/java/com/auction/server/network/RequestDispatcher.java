...
public class RequestDispatcher {
  ...
  private Response handleListItem(Request request, ClientHandler client) {
    ...
    switch (payload.category()) {
      ...
      case "ART" -> {
        ...
        extraData.put("authenticated", payload.authenticated());
        extraData.put("certificateId", payload.certificateId());
        extraData.put("dimensions", payload.dimensions());
      }
      case "VEHICLE" -> {
        extraData.put("vehicleType", payload.vehicleType());
        ...
        extraData.put("transmission", payload.transmission());
        extraData.put("color", payload.color());
        extraData.put("licensePlate", payload.licensePlate());
        extraData.put("hasValidRegistry", payload.hasValidRegistry());
      }
  ...
  private java.util.Map<String, String> extractItemAttributes(Item item) {
    ...
    if (item instanceof com.auction.model.Electronics e) {
      ...
      attributes.put("Đã chứng thực", a.isAuthenticated() ? "Có" : "Không");
      if (a.getCertificateId() != null && !a.getCertificateId().isBlank()) {
        attributes.put("Mã chứng thư", a.getCertificateId());
      }
      if (a.getDimensions() != null && !a.getDimensions().isBlank()) {
        attributes.put("Kích thước", a.getDimensions());
      }
    } else if (item instanceof com.auction.model.Vehicle v) {
      attributes.put("Loại xe", v.getVehicleType());
      attributes.put("Hãng xe", v.getMake());
      attributes.put("Dòng xe", v.getModel());
      attributes.put("Năm sản xuất", String.valueOf(v.getYear()));
      attributes.put("Số KM đã đi", String.valueOf(v.getMileage()) + " km");
      attributes.put("Nhiên liệu", v.getFuelType() != null ? v.getFuelType().getDisplay() : "N/A");
      attributes.put("Hộp số", "AUTOMATIC".equals(v.getTransmission()) ? "Tự động" : "Số sàn");
      attributes.put("Màu sắc", v.getColor());
      attributes.put("Biển số", v.getLicensePlate());
      attributes.put("Giấy tờ hợp lệ", v.isHasValidRegistry() ? "Có" : "Không");
    }
    return attributes;
  }
}
