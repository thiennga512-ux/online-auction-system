...
public class UserService {
  ...
  public void depositForBidder(String bidderId, double amount) {
    try {
      ...
      if (opt.isEmpty()) {
        throw DatabaseException.notFound("User", bidderId);
      }
      User user = opt.get();
      if (user instanceof Bidder bidder) {
        ...
        System.out.printf("[UserService] Nạp %.0f VND cho Seller: %s%n",
            amount, seller.getFullName());
      } else {
        throw new IllegalArgumentException("Chỉ Bidder hoặc Seller mới có thể nạp tiền");
      }
    } catch (Exception e) {
