...
import com.auction.dto.Dto.AuctionCardDto;
...
public class AuctionResultsController implements LifeCycleAwareController {
  ...
  @FXML
  private Label successRateValue;

  @FXML
  private TableView<AuctionResult> resultsTable;
  @FXML
  private TableColumn<AuctionResult, String> colEndTime;
  @FXML
  private TableColumn<AuctionResult, String> colAuctionName;
  @FXML
  private TableColumn<AuctionResult, String> colStatus;
  @FXML
  private TableColumn<AuctionResult, String> colReason;
  @FXML
  ...
  // ===== Throttle: ngăn gửi quá nhiều request GET_AUCTION_RESULT trong thời gian
  // ngắn =====
  ...
  private void loadAuctionResults() {
    ...
    if (now - last < MIN_REFRESH_INTERVAL_MS) {
      ...
      System.out.println(
          "[AuctionResultsController] Throttle: bỏ qua refresh, chưa đủ " + MIN_REFRESH_INTERVAL_MS + "ms từ lần cuối");
      return;
    }
  ...
  private void handleResponse(Response response) {
    ...
    if (action == ActionType.AUCTION_ENDED_BROADCAST) {
      System.out.println("[AuctionResultsController] Nhận AUCTION_ENDED_BROADCAST: " + response.getMessage());
    ...
    if (action == ActionType.GET_AUCTION_RESULTS) {
      Platform.runLater(() -> {
        ...
        List<AuctionCardDto> results = response.getDataAsList(AuctionCardDto.class);
   ...
   */
  private void populateTable(List<AuctionCardDto> results) {
    auctionResults.clear();
    ...
    for (AuctionCardDto dto : results) {
      ...
      try {
        LocalDateTime ldt = LocalDateTime.parse(dto.actualEndTime());
        displayEndTime = ldt.format(DISPLAY_FORMATTER);
      } catch (Exception e) {
        displayEndTime = dto.actualEndTime();
      }
      ...
      switch (rawStatus) {
        case "FINISHED" -> {
          // Có người thắng → thành công, không có → thất bại
          boolean hasWinner = dto.currentWinnerId() != null && !dto.currentWinnerId().isBlank();
          if (hasWinner) {
            statusText = "Thành Công";
            reasonText = "Người thắng: " + (dto.currentWinnerName() != null ? dto.currentWinnerName() : "N/A");
            successCount++;
          } else {
            statusText = "Thất Bại";
            reasonText = "Không có người đặt giá";
          }
        }
        case "CANCELLED" -> {
          statusText = "Bị Hủy";
          reasonText = "Phiên bị hủy bởi Admin";
        }
        case "REJECTED" -> {
          statusText = "Bị Hủy";
          reasonText = "Phiên bị từ chối bởi Admin";
        }
        default -> {
      ...
      auctionResults.add(new AuctionResult(displayEndTime, dto.itemName(), statusText, reasonText));
    }
