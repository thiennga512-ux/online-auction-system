...
public class AuctionDetailController implements LifeCycleAwareController {
  ...
  @FXML
  private ListView<String> bidHistoryList; // Vẫn giữ cho tương thích, dùng bidHistoryTable là chính
  @FXML
  private Button notifyButton;
  @FXML
  private ProgressBar timeProgressBar;
  @FXML
  private Region livePulseDot;

  // Bid panel
  @FXML
  private VBox bidPanel;
  @FXML
  private Label minBidHintLabel;
  @FXML
  private TextField bidAmountField;
  @FXML
  private Button submitBidBtn;
  @FXML
  private Label bidStatusLabel;
  @FXML
  private Button preset1Btn;
  @FXML
  private Button preset2Btn;
  @FXML
  private Button preset3Btn;
  @FXML
  private Button preset4Btn;
  @FXML
  private Button preset5Btn;
  ...
  @FXML
  private TableColumn<BidRow, String> timeColumn;
  @FXML
  private TableColumn<BidRow, String> bidderColumn;
  @FXML
  private TableColumn<BidRow, String> amountColumn;
  @FXML
  private TableColumn<BidRow, String> typeColumn;
  @FXML
  private Label bidTableStatusLabel;
  ...
  @FXML
  private TextField autobidCeilingField;
  @FXML
  private TextField autobidIncrementField;
  @FXML
  private Button activateAutobidBtn;
  @FXML
  ...
  public Dto.AuctionCardDto getCurrentSessionData() {
  ...
  private void setupBidHistoryTable() {
    ...
    // để tránh lỗi "Unable to coerce TableView.CONSTRAINED_RESIZE_POLICY to
    // interface Callback"
  ...
  private void buildSpecsGrid(Dto.AuctionCardDto session) {
    if (specsGrid == null || session == null)
      return;

    specsGrid.getChildren().clear();
    specsGrid.getRowConstraints().clear();

    java.util.List<String[]> specList = new java.util.ArrayList<>();
    specList.add(new String[] { "📦 Tên sản phẩm", session.itemName() != null ? session.itemName() : "N/A",
        "spec-value spec-value-bold-white" });
    specList.add(new String[] { "📂 Danh mục", session.category() != null ? session.category() : "N/A", null });
    specList.add(new String[] { "👤 Người bán", session.sellerName() != null ? session.sellerName() : "N/A", null });

    if (session.itemAttributes() != null) {
      for (java.util.Map.Entry<String, String> entry : session.itemAttributes().entrySet()) {
        String value = entry.getValue() != null && !entry.getValue().isBlank() ? entry.getValue() : "N/A";
        specList.add(new String[] { "✨ " + entry.getKey(), value, null });
      }
    }

    specList.add(new String[] { "💰 Giá khởi điểm", formatMoney(session.startingPrice()) + " VND", null });
    specList.add(new String[] { "📊 Bước giá tối thiểu", formatMoney(session.minIncrement()) + " VND", null });
    specList.add(new String[] { "💎 Giá hiện tại", formatMoney(session.currentPrice()) + " VND",
        "spec-value spec-value-emerald" });

    // --- Populate GridPane từ specList ---
    int dataRowCount = specList.size();

    for (int i = 0; i < dataRowCount; i++) {
      int row = i;
      String[] data = specList.get(i);

      // ===== CỘT 1: Icon + Label =====
      Label labelNode = new Label(data[0]);
      labelNode.getStyleClass().add("spec-label");
      labelNode.setMaxWidth(Double.MAX_VALUE);
      GridPane.setConstraints(labelNode, 0, row);

      // ===== CỘT 2: Giá trị =====
      Label valueNode = new Label(data[1]);
      String styleClassStr = data[2];
  ...
  private void updateChartData() {
    if (priceChart == null || chartPlaceholder == null)
      return;
  ...
  private void handleActivateAutobid() {
    ...
    if (increment < minIncrement) {
      setAutobidStatus("Bước giá phải lớn hơn hoặc bằng bước giá tối thiểu (" + formatMoney(minIncrement) + " VND).",
          true);
      return;
    }
  ...
  private void activateAutobid(double ceiling, double increment) {
    ...
    Platform.runLater(() -> {
      ...
      if (activateAutobidBtn != null) {
        ...
        activateAutobidBtn.setStyle(
            "-fx-background-color: #ef4444; -fx-text-fill: white; -fx-font-size: 16px; -fx-font-weight: bold; -fx-background-radius: 10; -fx-padding: 16 32; -fx-cursor: hand; -fx-min-height: 54;");
      }
      if (autobidCeilingField != null)
        autobidCeilingField.setDisable(true);
      if (autobidIncrementField != null)
        autobidIncrementField.setDisable(true);
      setAutobidStatus("⏳ Đang xử lý...", false);
    });
  }
  ...
  private void deactivateAutobid() {
    ...
    Platform.runLater(() -> {
      ...
      if (autobidCeilingField != null)
        autobidCeilingField.setDisable(false);
      if (autobidIncrementField != null)
  ...
  private void handleBidHistory(Response response) {
    ...
    Platform.runLater(() -> {
      ...
      int selectedIndex = auctionTabPane != null && auctionTabPane.getSelectionModel().getSelectedItem() != null
          ? auctionTabPane.getTabs().indexOf(auctionTabPane.getSelectionModel().getSelectedItem())
          : -1;
      if (selectedIndex == 2) {
        updateChartData();
      }
  ...
  private void handleNewBidEvent(Response response) {
    ...
    if (event == null || !sessionId.equals(event.sessionId()))
      return;
    ...
    Platform.runLater(() -> {
      ...
      String displayType = event.bidType() != null && event.bidType().equals("AUTO") ? "Tự động" : "Đặt thủ công";
      ...
      // Update Tab 2: Giá hiện tại
      if (specsGrid != null) {
        for (Node node : specsGrid.getChildren()) {
          if (node instanceof Label label) {
            if (label.getStyleClass().contains("spec-value-emerald")) {
              label.setText(formatMoney(event.amount()) + " VND");
            }
          }
        }
      }
  ...
  private void handleAutoBidResponse(Response response) {
    Platform.runLater(() -> {
      if (response.isSuccess()) {
        if (autobidActive) {
          setAutobidStatus("✅ AutoBid đã kích hoạt thành công! Hệ thống sẽ tự động đặt giá thay bạn.", false);
        } else {
        ...
        if (autobidActive) {
          ...
          if (activateAutobidBtn != null) {
            activateAutobidBtn.setText("KÍCH HOẠT AUTOBID");
            activateAutobidBtn.setStyle(null);
            activateAutobidBtn.getStyleClass().add("autobid-activate-btn");
          }
          if (autobidCeilingField != null)
            autobidCeilingField.setDisable(false);
  ...
  private void updateViewerCount(List<Bid> bids) {
    ...
    bids.forEach(b -> {
      if (b.getBidderName() != null)
        bidders.add(b.getBidderName());
    });
  ...
  private void updateBidPanel() {
    ...
    if (minBidHintLabel != null) {
      if (!isRunning) {
        ...
        minBidHintLabel.setText("Đăng nhập bằng tài khoản mua/bán để đặt giá");
      } else {
  ...
  private void setPresetDisable(boolean disabled) {
    if (preset1Btn != null)
      preset1Btn.setDisable(disabled);
    if (preset2Btn != null)
      preset2Btn.setDisable(disabled);
    if (preset3Btn != null)
      preset3Btn.setDisable(disabled);
    if (preset4Btn != null)
      preset4Btn.setDisable(disabled);
    if (preset5Btn != null)
      preset5Btn.setDisable(disabled);
  }
  ...
  @FXML
  private void handlePreset4() {
    addPreset(5_000_000);
  }

  @FXML
  private void handlePreset5() {
    addPreset(10_000_000);
  }
