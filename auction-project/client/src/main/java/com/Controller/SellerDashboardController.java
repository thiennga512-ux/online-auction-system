...
public class SellerDashboardController implements LifeCycleAwareController {
  ...
  public void initialize() {
    ...
    fuelTypeComboBox.setItems(FXCollections.observableArrayList("GASOLINE", "DIESEL", "ELECTRIC", "HYBRID"));
  ...
  private void handleGetMyItemsResponse(Response r) {
    if (r.isSuccess()) {
      ...
      if (itemComboBox != null) {
        // Chỉ cho phép đăng ký đấu giá sản phẩm còn hàng (AVAILABLE)
        ...
        // Nếu item đang được chọn không còn AVAILABLE nữa thì xoá selection
        Dto.ItemResponse selected = itemComboBox.getValue();
        if (selected != null && !"AVAILABLE".equals(selected.status())) {
